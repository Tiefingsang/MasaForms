...
<body class="bg-gray-100 font-sans antialiased"
    ...
    <div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8" x-data="publicForm()" x-init="init({{ $form->id }})">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white shadow-xl rounded-lg overflow-hidden"
                ...
                <!-- Messages -->
                <div x-show="message" x-cloak class="px-6 py-4" :class="messageType === 'success' ? 'bg-green-50' : 'bg-red-50'">
                                ...
                                <div class="space-y-2">
                                    <template x-for="(option, idx) in field.options" :key="idx">
                                        <div class="flex items-center">
                                            ...
                                            <label :for="'field_' + field.id + '_' + idx" class="ml-3 text-sm text-gray-700" x-text="option"></label>
                                        </div>
                                    </template>
                                </div>
                            </template>
    ...
    function publicForm() {
        return {
            ...
            async loadForm() {
                try {
                    const response = await fetch(`/api/forms/${this.formId}/public`);
                    const data = await response.json();
                    this.fields = data.fields.sort((a, b) => a.order - b.order);
            ...
            isFieldVisible(field) {
                // Logique de conditions (à implémenter selon vos besoins)
                return true;
            },
                ...
                this.fields.forEach(field => {
                    if (field.is_required) {
                        if (field.type === 'checkbox') {
                            ...
                            if (this.responses[field.id] && this.responses[field.id].trim() !== '') {
                                filled++;
                            }
                        }
                    }
            ...
            async submitForm() {
                ...
                try {
                    ...
                    this.fields.forEach(field => {
                        if (field.type === 'checkbox') {
                            formData.responses[field.id] = this.checkboxValues[field.id] || [];
                        } else {
                            formData.responses[field.id] = this.responses[field.id] || '';
                        }
                    });
